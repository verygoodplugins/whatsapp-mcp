...
- **Per-chat Access Control**: Independently allow new messages, saved history, and outbound actions for each conversation
...
2. **Start the WhatsApp bridge**
   ...
   On first start, the bridge prints and stores `.bridge-token` for
   MCP-to-bridge calls. It also creates `.admin-token` for the local
   permissions panel, but does not print that more-privileged secret. Scan the
   QR code with WhatsApp on your phone to authenticate.

3. **Choose which chats the MCP may access**

   Open <http://127.0.0.1:8080/admin/> and sign in with the token from
   `whatsapp-bridge/store/.admin-token`. New and unconfigured conversations
   are blocked by default. For each chat, independently choose whether the MCP
   may read new messages, read the saved history, and send messages or actions.
   A separate global switch can allow the MCP to initiate new individual chats.
...
An unknown individual phone number is accepted only when **Start new individual
conversations** is enabled in the local Bridge panel.
...
- `chat_jid` (optional, recommended): JID of the chat; required when the same message ID exists in multiple chats
...
| `WHATSAPP_ADMIN_TOKEN` | generated by the bridge as `store/.admin-token` | Separate secret for the local per-chat permissions panel |
...
| `WHATSAPP_MCP_LEGACY_ALLOW_ALL` | unset (`false`)                    | Temporary opt-in for an older bridge without the permissions table |

### Per-chat MCP access

The bridge serves a local administration panel at
<http://127.0.0.1:8080/admin/>. It uses a dedicated admin token, generated at
`whatsapp-bridge/store/.admin-token` with owner-only permissions. This secret
is intentionally different from `.bridge-token`: the MCP server needs the
bridge token, but must never receive the admin token that can expand its own
access.

The panel is available in **Portuguese (Brazil)** and **English**. Use the
language selector on either the login screen or the authenticated header; the
non-sensitive preference is remembered locally in the browser. Translations
are kept in independent catalogs under `whatsapp-bridge/admin/i18n/`, and tests
require both catalogs to expose the same complete set of keys.

The three permissions are independent:

- **Read new messages** exposes messages whose timestamp is at or after the
  moment the permission was enabled. Disabling and later re-enabling it leaves
  the messages from the disabled interval private.
- **Read saved history** exposes messages whose WhatsApp timestamp is before
  the permission was enabled. History synchronized later is evaluated by the
  original WhatsApp message timestamp, so an older synchronized message joins
  that historical window.
- **Send** allows text, files, voice messages, reactions, and typing indicators.
  It does not imply either read permission.

The panel also has a disabled-by-default **Start new individual conversations**
setting. When enabled, the MCP may send an initial text or media message to a
valid phone number that is not yet present in the local `chats` table. A
successful first send creates that chat with **full access**: read new messages,
read saved history, and send. The global setting never overrides an existing
chat's explicit deny, does not allow unknown group JIDs, and cannot be used for
quoted replies, reactions, or typing indicators.

WhatsApp timestamps commonly have one-second precision. To keep the permission
boundary conservative, the activation second itself is excluded from both a
new-message grant and a history grant.

The permissions table is created by the updated bridge. From that point on,
missing rows—including newly discovered conversations—deny all normal MCP
access. The optional first-send path described above is the only exception and
immediately creates a full-access row after success. Chat lists, contact search,
previews, message context, downloads, and outbound bridge endpoints all enforce
the policy. The panel lists chat names and JIDs but never returns message
content; explicitly configured send-only chats expose no message preview.

The MCP server also fails closed when the permissions table is missing. If an
older bridge must be kept running briefly during an upgrade, setting
`WHATSAPP_MCP_LEGACY_ALLOW_ALL=true` restores the previous unrestricted
behavior. Treat that flag as a temporary migration escape hatch and remove it
as soon as the updated bridge has started once.

Message, media, and reaction webhooks use the same read windows. A blocked
conversation is still synchronized and stored locally, but its payload and
media bytes are not forwarded to `WEBHOOK_URL`.

Treat these controls as an application boundary, not an operating system
sandbox: a process or coding agent with direct filesystem access to
`messages.db`, downloaded media, or the admin secret can bypass them. Revoking
access blocks future MCP and webhook exposure but cannot retract content a
model already received. Protect the store with OS permissions and do not
expose it to untrusted processes.
