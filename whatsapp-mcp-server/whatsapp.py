...
def _bridge_post(path: str, payload: dict[str, Any]) -> requests.Response:
    """POST to the bridge REST API, auto-starting the bridge if it is down.

    A ConnectionError means nothing is listening — typically the bridge was
    never started. bridge.ensure_bridge_running() launches it (when enabled
    and local) and the request is retried once. Headers are rebuilt for the
    retry so a token generated by a first-ever bridge start is picked up.
    """
    url = f"{WHATSAPP_API_BASE_URL}{path}"
    try:
        return requests.post(url, json=payload, headers=_bridge_headers())
    except requests.ConnectionError:
        ok, detail = bridge.ensure_bridge_running()
        if not ok:
            raise requests.ConnectionError(f"WhatsApp bridge is not running ({detail})") from None
        return requests.post(url, json=payload, headers=_bridge_headers())
...
def send_reaction(
    ...
    try:
        ...
        response = _bridge_post("/react", payload)
