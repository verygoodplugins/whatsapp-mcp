...
| `WHATSAPP_ADMIN_TOKEN` | generated by the bridge as `store/.admin-token` | Separate secret for the local per-chat permissions panel; never expose it to the MCP server |
...
| `WHATSAPP_MCP_LEGACY_ALLOW_ALL` | unset (`false`) | Temporary opt-in for legacy allow-all behavior when an older bridge has no permissions table |
